...
void *SendMessage () {
	...
	while (online) {
		...
		} else if (strcmp(aux, "CREATEG") == 0) {
			snprintf(buf, MAX_LINE, "%d %s", create_gmsg, str+8);		
		
		} else if (strcmp(aux, "JOING") == 0) {
			snprintf(buf, MAX_LINE, "%d %s", join_gmsg, str+6);
		
		} else if (strcmp(aux, "SENDG") == 0) {
			fdbk = 1;
			id = hash(str);
			snprintf(idstr, 10, "%d", id);
			snprintf(buf, MAX_LINE, "%d %d %s", new_gmsg, id, str+6);
